    ...
    <>
      ...
      <section className="container shop-layout">
        ...
        <div className="shop-results">
          <div className="shop-toolbar"><label><Search size={18} /><input value={query} onChange={(e) => setParam('q', e.target.value)} placeholder="Search the collection" /></label><button className="filter-trigger" onClick={() => setFiltersOpen(true)}><SlidersHorizontal size={18} /> Filters</button><span>{filtered.length} {filtered.length === 1 ? 'object' : 'objects'}</span><select value={sort} onChange={(e) => setParam('sort', e.target.value, 'featured')} aria-label="Sort products"><option value="featured">Featured first</option><option value="price-low">Price: low to high</option><option value="price-high">Price: high to low</option><option value="rating">Highest rated</option></select></div>
