    ...
    <>
      ...
      <section className="container shop-layout">
        ...
        <div className="shop-results">
          ...
          {filtered.length ? <div className="product-grid shop-grid">{filtered.map((product) => <ProductCard product={product} key={product.id} />)}</div> : <div className="no-results"><h2>No matching products</h2><p>That search is not in the current catalog.</p><button className="button dark" onClick={() => setParams({})}>View all products</button></div>}
        </div>
      </section>
    </>
  )
